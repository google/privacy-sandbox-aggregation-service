...
func TestReachUint64TupleDpfGenEvalAtRange(t *testing.T) {
	os.Setenv("GODEBUG", "cgocheck=2")
	keyBitSize := 128
	startLevel, endLevel := 101, 102

	params, err := GetDefaultTupleDPFParametersFullHierarchy(keyBitSize)
	if err != nil {
		t.Fatal(err)
	}
	...
	expanded1, err := EvaluateReachTupleAtRange(evalCtx1, startLevel, endLevel)
	if err != nil {
		t.Fatal(err)
	}
	...
	expanded2, err := EvaluateReachTupleAtRange(evalCtx2, startLevel, endLevel)
	if err != nil {
		t.Fatal(err)
	}
	...
	wantNonZeroIndex := nonZeroIndex >> (keyBitSize - endLevel - 1)
	wantSize := 1 << (endLevel - startLevel + 1)
