...
func getAEADForKMS(ctx context.Context, keyURI, credentialPath string) (tink.AEAD, error) {
	...
	if credentialPath != "" {
		gcpclient, err = gcpkms.NewClient(ctx, keyURI, gcpkms.WithGoogleAPIClientOptions(option.WithCredentialsFile(credentialPath)))
	} else {
		gcpclient, err = gcpkms.NewClient(ctx, keyURI)
	}
	if err != nil {
		return nil, err
	}
