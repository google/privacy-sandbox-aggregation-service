...
func GetDefaultDPFParameters(keyBitSize int) ([]*dpfpb.DpfParameters, error) {
	...
	return []*dpfpb.DpfParameters{
		{LogDomainSize: int32(keyBitSize), ElementBitsize: incrementaldpf.DefaultElementBitSize},
	}, nil
...
func (fn *expandDpfKeyAtFn) ProcessElement(ctx context.Context, evalCtx *dpfpb.EvaluationContext, emitVec func(*expandedVec)) error {
	if 0 <= evalCtx.PreviousHierarchyLevel {
	...
	vecSum, err := incrementaldpf.EvaluateAt64Unsafe(evalCtx.Parameters, 0, fn.cBuckets, fn.cBucketsLength, evalCtx.Key)
	if err != nil {
		return err
	}
	emitVec(&expandedVec{SumVec: vecSum})
