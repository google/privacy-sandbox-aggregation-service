...
func CreateReachUint64TupleDpfParameters(logDomainSize int32) *dpfpb.DpfParameters {
	...
	// http://github.com/google/distributed_point_functions/dpf/distributed_point_function.proto;l=104;rcl=477166564
...
// EvaluateReachTupleAtRange expands the given DPF key into ReachTuple buckets.
func EvaluateReachTupleAtRange(evalCtx *dpfpb.EvaluationContext, startLevel, endLevel int) ([]*ReachTuple, error) {
	bEvalCtx, err := proto.Marshal(evalCtx)
	if err != nil {
		return nil, err
	}
	...
	outExpanded := C.struct_CReachTupleVec{}
	errStr := C.struct_CBytes{}
	status := C.CEvaluateAtRangeReachTuple(&cEvalCtx, C.int(startLevel), C.int(endLevel), &outExpanded, &errStr)
	defer freeCBytes(cEvalCtx)
	defer C.free(unsafe.Pointer(outExpanded.vec))
	defer freeCBytes(errStr)
	if status != 0 {
...
// GetDefaultTupleDPFParametersFullHierarchy generats the DPF parameters for Reach tuples with uint64 elements for all possible prefixes.
func GetDefaultTupleDPFParametersFullHierarchy(keyBitSize int) ([]*dpfpb.DpfParameters, error) {
	if keyBitSize <= 0 {
		return nil, fmt.Errorf("keyBitSize should be positive, got %d", keyBitSize)
	}
	allParams := make([]*dpfpb.DpfParameters, keyBitSize)
