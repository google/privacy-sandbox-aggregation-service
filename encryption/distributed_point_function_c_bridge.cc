...
int EvaluateAtRangeTupleReachIntNodN(
    std::unique_ptr<DistributedPointFunction> dpf,
    EvaluationContext& eval_context, int start_level, int end_level,
  ...
  if (start_level > 0) {
    result = dpf->EvaluateAt<ReachTuple<ReachIntModN>>(
        start_level - 1, std::vector<absl::uint128>{0}, eval_context);
    if (!result.ok()) {
      StrToCBytes(result.status().message(), out_error);
      return result.status().raw_code();
    }
  ...
  result = dpf->EvaluateAt<ReachTuple<ReachIntModN>>(
      end_level, evaluation_points, eval_context);
  if (!result.ok()) {
    StrToCBytes(result.status().message(), out_error);
    return result.status().raw_code();
  }
...
int EvaluateAtRangeTupleUint64(std::unique_ptr<DistributedPointFunction> dpf,
                               EvaluationContext& eval_context, int start_level,
                               int end_level,
                               std::vector<absl::uint128>& evaluation_points,
                               struct CReachTupleVec* out_vec,
                               struct CBytes* out_error) {
  ...
  result = dpf->EvaluateAt<ReachTuple<uint64_t>>(end_level, evaluation_points,
                                                 eval_context);
  if (!result.ok()) {
    StrToCBytes(result.status().message(), out_error);
    return result.status().raw_code();
...
// Get all the indices represented by the start/end levels, prefixed and
// suffixed with zeros.
std::vector<absl::uint128> GetEvaluationPoints(int start_level, int end_level) {
  std::vector<absl::uint128> points(1 << (end_level - start_level + 1));
...
int CEvaluateAtRangeReachTuple(const struct CBytes* in_context, int start_level,
                               int end_level, struct CReachTupleVec* out_vec,
                               struct CBytes* out_error) {
  EvaluationContext eval_context;
  if (!eval_context.ParseFromArray(in_context->c, in_context->l)) {
    StrToCBytes("fail to parse EvaluationContext", out_error);
  ...
  if (!dpf.ok()) {
    ...
    for (int i = 0; i < parameters.size(); i++) {
      std::cout << "" << parameters[i].DebugString() << std::endl;
    }
    return dpf.status().raw_code();
  }

  std::vector<absl::uint128> evaluation_points =
      GetEvaluationPoints(start_level, end_level);

  if (use_int.value()) {
    return EvaluateAtRangeTupleUint64(std::move(dpf.value()), eval_context,
                                      start_level, end_level, evaluation_points,
                                      out_vec, out_error);
  }
  return EvaluateAtRangeTupleReachIntNodN(
      std::move(dpf.value()), eval_context, start_level, end_level,
      evaluation_points, out_vec, out_error);
}
