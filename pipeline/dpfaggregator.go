...
func (fn *expandDpfKeyFn) Setup() {
	...
	fn.cPrefixes, fn.cPrefixesLength = incrementaldpf.CreateCPrefixesUnsafe(fn.ExpandParams.Prefixes[0])
}

func (fn *expandDpfKeyFn) Teardown() {
	incrementaldpf.FreeCPrefixes(fn.cPrefixes)
}
...
func (fn *expandDpfKeyFn) ProcessElement(ctx context.Context, evalCtx *dpfpb.EvaluationContext, emitVec func(*expandedVec)) error {
	...
	vecSum, err := incrementaldpf.EvaluateUntil64Unsafe(int(fn.ExpandParams.Levels[0]), fn.cPrefixes, fn.cPrefixesLength, evalCtx)
	if err != nil {
		return err
	}
