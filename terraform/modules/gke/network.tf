...
# and so are created for each region
...
resource "google_compute_subnetwork" "subnet" {
  name    = "${var.environment}-${var.settings.region}-instances"
...
resource "google_compute_router" "router" {
  name    = "${var.environment}-${var.settings.region}-router"
  network = google_compute_network.network.self_link
  region  = var.settings.region
}
...
resource "google_compute_router_nat" "nat" {
  name   = "${var.environment}-${var.settings.region}-nat"
