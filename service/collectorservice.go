...
// Shutdown shutdown function used in http.Server.RegisterOnShutdown to close channel and flush
// remaining reports
...
func (brw *bufferedReportWriter) start(ctx context.Context, reportsCh <-chan *AggregationReport) {
	...
	var batch []*AggregationReport
	brw.wg.Add(1)
	go func() {
		for report := range reportsCh {
			batch = append(batch, report)
			if len(batch) == brw.batchSize {
				brw.writeReports(ctx, batch)
				batch = []*AggregationReport{}
			}
		}
...
func (brw *bufferedReportWriter) writeReports(ctx context.Context, batch []*AggregationReport) {
	...
	origins := make([]string, 0, len(reportBatches))
	for k := range reportBatches {
		origins = append(origins, k)
	}

	batchKey := strings.Join(origins, "+")
